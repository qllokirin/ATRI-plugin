...
export class atri extends plugin {
    ...
    async atri(e) {
        ...
        //注册账号开通翻译api去这里找到你的appid和key填到下面
        //每月5w免费额度  应该用不完吧 后面再看看有无其他免费的翻译api
        let appid = ''
        let key = ''
        if (appid == false || key == false) {
            logger.error("未填入百度翻译api的appid和key\n请按照https://github.com/70loKirin/ATRI-plugin进行配置")
            this.e.reply("未填入百度翻译api的appid和key\n请按照https://github.com/70loKirin/ATRI-plugin进行配置")
            return false
        }
        ...
        if(res.trans_result == undefined){
            logger.error(res)
            this.e.reply("错误码："+res.error_code)
            this.e.reply("百度翻译api出错，请到http://api.fanyi.baidu.com/doc/21搜索你的错误码")
            return false
        }
        console.log(res)
